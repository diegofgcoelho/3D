...
MatchSet::MatchSet(FrameSet frames) {
...
int MatchSet::generateKeyPointsAndDescriptors() {
	...
	for(int i = 0; i < this->frameSet.getNFrames(); i++){
...
int MatchSet::generateMatches(unsigned int nNeighbors) {
	...
	for(int i = 0; i < this->frameSet.getNFrames(); i++){
		...
		unsigned int dsize = min(this->frameSet.getNFrames()-1-i, static_cast<int>(nNeighbors));
