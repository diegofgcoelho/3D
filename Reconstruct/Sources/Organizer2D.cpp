...
int Organizer2D::organizePoints(vector<vector<vector<DMatch> > > matches, vector<vector<KeyPoint> > keypoints) {
	...
	//Removing the points that happens not so often
	this->cleanPoints();
...
void Organizer2D::cleanPoints() {
	...
	for(unsigned int i = 0; i < this->points.size(); i++){
		...
		//New matrix
		Mat_<double> mat(2, newSize, -1);
		//Index counting the position in the new matrix
		unsigned int index = 0;
		for(unsigned int j = 0; j < fullLength; j++){
			if(occurrences[j] > this->minOcc){
				mat(0,index) = matO(0,j);
				mat(1,index) = matO(1,j);
				index++;
			}
		}
		//Copying the new matrix to the vector of points
		this->points.at(i) = mat;
	}
}
